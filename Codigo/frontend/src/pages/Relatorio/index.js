...
export default function Relatorio() {
  ...
  function calcularLucroTotal() {
    ...
    listInvestimentos.forEach(investment => {
      lucro += (investment.preco_venda - investment.preco_compra) * investment.quantidade
    })
   ...
   <>
      <header>
        <div className="px-3 py-2 sticky-top container">
          <div className="d-flex flex-wrap align-items-center justify-content-center" style={{ height: '100%' }}>
            <Link to="/" style={{ display: 'flex', alignItems: 'center' }}>
              <img className="mx-auto" style={{ maxWidth: '25%', maxHeight: '100%' }} src={financeLabLogo} alt="Logo" />
            </Link>
          </div>
        </div>
      </header>
      ...
      <Table striped bordered hover>
        ...
        <tbody>
          {listInvestimentos.map(investment => (
            <tr key={investment.id}>
              ...
              <td>
                {
                  ((investment.preco_venda - investment.preco_compra) * investment.quantidade).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })
                }&nbsp;

                ({((investment.preco_venda/investment.preco_compra - 1) * 100).toFixed(2)})%
              </td>
            </tr>
          ))}
        ...
        <h2>
          Total de Lucro/Prejuízo: R$ {calcularLucroTotal().toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })} 
        </h2>
      </Table>
   </>
  )
